...
class LocationActivity : AppCompatActivity(), OnMapReadyCallback  {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val loc = getLocationFromAddress(this, K.address) as LatLng

        // Add a marker in Sydney and move the camera
        val office = LatLng(loc.latitude, loc.longitude)
        mMap.addMarker(MarkerOptions().position(office).title(K.address))
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(office, 15F))

    }

    private fun loadInGoogleMaps() {
        intent = Intent(ACTION_VIEW, Uri.parse("google.navigation:q=${K.address}"))
